function makePrintScreen(){
	...
	var html = new DOMParser().parseFromString("<html>"+document.head.outerHTML+document.body.outerHTML+"</html>", "text/html");
	var head = html.head;
	var body = html.body;
	removeScripts(head, "script");
	removeScripts(body, "script");
	removeScripts(head, "template");
	removeScripts(body, "template");
	removeScripts(head, "link");
	removeScripts(body, "link");
	...
	var w = window.open('', "", "location=no,menubar=no,resizable=no,status=no,titlebar=no,toolbar=no");
	w.document.open();
	w.document.write("<!doctype html>"+"<html>"+head.outerHTML+body.outerHTML+"</html>");
	w.document.close();
